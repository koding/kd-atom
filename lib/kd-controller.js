...
export default class KDController extends kd.Object {
  ...
  setPowerState ({ machine, state, element }) {
    ...
    kdRun(`machine ${cmd} ${machine.alias}`)
      ...
      .catch(err => {
        ...
        element.removeClass('has-progress')
  ...
  closeMountSettings () {
    if (this.mountSettings) {
      this.mountSettings.destroy()
      this.mountSettings = null
    }
  }

  showMountSettingsView (mounts, options = {}) {
    this.mountSettings = atom.workspace.addBottomPanel({
      className: 'kd-machine-selector',
      item: new MountSettings({
        mounts,
        options: {
          ...options,
          onClose: () => this.mountSettings.destroy()
        }
      }),
      visible: true,
      priority: 201
  ...
  async showMounts () {
    ...
    this.showMountSettingsView(mounts, {
      onClose: () => {
        this.hideLoading()
        this.closeMountSettings()
      },
      onOpen: ({ mount }) => {
