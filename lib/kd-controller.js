...
import StackSettings from './views/stack-settings'
...
import getStacks from './utils/get-stacks'
...
export default class KDController extends kd.Object {
  ...
  showStackSettingsView (stacks, machines, options) {
    this.stackPanel = atom.workspace.addBottomPanel({
      className: 'kd-machine-selector',
      item: new StackSettings({
        stacks,
        machines,
        options,
      }),
  ...
  async showStacks () {
    log('show stacks')
    this.showLoading('Loading stacks…')

    let stacks, machines
    try {
      stacks = await getStacks()
      machines = await getMachines()
      this.getData().stacks = stacks
      this.getData().machines = machines
    } catch (e) { return this._err(e) }

    this.hideLoading()

    this.showStackSettingsView(stacks, machines, {
      onClose: () => {
        this.hideLoading()
        this.destroyPanel('stack')
      },
      onReload: () => {
        this.showStacks()
        this.destroyPanel('stack')
