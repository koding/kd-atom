...
export default class KDController extends kd.Object {
  ...
  constructor(serializedState = {}) {
    ...
    this.notificationCtrl = new NotificationController
    this.panel = null

    this.teamSelector = null
    this.machineSelector = null
    this.mountSelector = null
  }

  showLoading(message, icon, duration) {
    log('loading', { message, icon, duration })

    this.cleanScreen()

    return this.notificationCtrl.info({
      title: 'Loading',
      message,
      icon,
      duration
  ...
  showError(err) {
    ...
    return this.notificationCtrl.error({
      message: err.message,
      stack: err.stack
    })
  }
  ...
  cleanScreen() {
    [
      this.mountSelector,
      this.machineSelector,
      this.teamSelector
    ].forEach(selector => selector && selector.hide())
  }

  showPanel({ className, view, events }) {
    ...
    Object.keys(events, (eventName, handler) => {
      view.emitter.on(eventName, handler)
    })
  ...
  async showMounts (showMessage = true) {
    ...
    view.emitter.on('mountSelector-confirmed', mount => {
      ...
      this._showTreeView()
      this.notificationCtrl.success({
        title: 'Success!',
        message: 'Your machine has successfully mounted and added as a project.'
      })
    })
  }
    ...
    view.emitter.on('machineSelector-confirmed', machine => {
      ...
      let panel = this.showPrompt({
        ...
        callback: (err, inputs) => {
          ...
          this.showLoading(`
            Mounting **${remoteAddress}** to **${inputs.localPath}**, please wait…
          `)
          execa('kd', ['machine', 'mount', remoteAddress, inputs.localPath]).then(result => {
            this.showLoading(`Machine mounted, preparing the mount please wait…`, 'check', 2000)
            ...
            this.showLoading('Machine could not be mounted, rolling back…', 'x', 2000)
            error(e)
          })
        }
      })
